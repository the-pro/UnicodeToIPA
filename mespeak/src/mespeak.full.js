...
// public API
	...
	// unlock mobile audio
	var mobileAudioUnlocked = false;
	function mobileAudioUnlocker (event) {
		...
		if (AudioAPI) {
			...
			var buffer = audioContext.createBuffer(1, 1, 22050),
				source = audioContext.createBufferSource();
			audioContext.buffer = buffer;
			audioContext.connect(audioContext.destination);
			(source.noteOn || source.start)(0);
		}
		mobileAudioUnlocked = true;
		self.removeEventListener('touchstart', mobileAudioUnlocker);
	}

	loadCore();
	resolveAudioApi();
	window.addEventListener('unload', unloadHandler, false);
	if (isMobile && typeof self.ontouchstart !== undefined) self.addEventListener('touchstart', mobileAudioUnlocker, false);
